...
class Test_fs_mkfs:
  ...
  def test_hasno_foodir(self):
    print "The filesystem should not have a \"foo\" directory."
    py.test.raises(OSError, "self.fs.fs_getattr('/foo')")

  def test_has_dotdir(self):
    print "The filesystem should have a .utopfs directory."
    statbuf = self.fs.fs_getattr("/.utopfs")
    assert S_ISDIR(statbuf[ST_MODE])

    print "The filesystem should have a version file."
    statbuf = self.fs.fs_getattr("/.utopfs/version")
    assert S_ISREG(statbuf[ST_MODE])

    print "The version file should contain a version string."
  ...
  def test_can_create_file(self):
    print "We should be able to create a file."
